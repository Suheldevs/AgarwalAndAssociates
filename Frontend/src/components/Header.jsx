...
export default function Header() {
  ...
  return (
    <header className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
            ...
            <Link to="/" className="flex items-center">
              ...
              <div className="ml-2">
                <h1 className="text-xl font-bold text-gray-800 leading-none">
                  AGARWAL <span className="text-gray-500 font-light">&</span>
                </h1>
                <p className="text-xs text-gray-600 font-light tracking-widest">ASSOCIATES</p>
              </div>
            </Link>
          </div>
                ...
                <li key={index} className="relative group">
                  {item.isDropdown ? (
                    <div>
                      ...
                      <div className={`absolute top-full left-0 mt-2 bg-white shadow-lg rounded-md py-2 w-48 transform transition-all origin-top ${servicesOpen ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0'}`}>
      ...
      {/* Mobile Navigation */}
      <div className={`lg:hidden bg-white transition-all duration-300 overflow-hidden ${isMenuOpen ? 'max-h-screen shadow-lg' : 'max-h-0'}`}>
              ...
              <li key={index}>
                {item.isDropdown ? (
                  <div>
                    ...
                    <div className={`pl-4 space-y-2 mt-2 transition-all ${servicesOpen ? 'block' : 'hidden'}`}>
                      {item.dropdownItems.map((dropdownItem, idx) => (
                        <Link 
                          key={idx}
                          to={dropdownItem.path} 
